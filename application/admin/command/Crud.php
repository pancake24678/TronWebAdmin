    ...
    protected function execute(Input $input, Output $output)
    {
        ...
        try {
            ...
            if ($recyclebinHtml) {
                ...
                $recyclebinTitleJs = $recyclebinList;
...
EOD;
    ...
    protected function getJsColumn($field, $datatype = '', $extend = '', $itemArr = [], $fieldConfig = [])
    {
        ...
        $noSearchFiles = ['file$', 'files$', 'image$', 'images$', '^weigh$','switch$','tags$','json$','content$','^createtime$', '^updatetime$'];
        if (preg_match("/" . implode('|', $noSearchFiles) . "/i", $field)) {
            $html .= ", operate: false";
        } elseif (in_array($datatype, ['varchar'])) {
            $html .= ", operate: 'LIKE'";
        }elseif (in_array($datatype, ['date', 'datetime']) || $formatter === 'datetime') {
